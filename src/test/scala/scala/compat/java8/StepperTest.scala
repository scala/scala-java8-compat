...
class StepperTest {
  class IncStepperA(private val size0: Long) extends NextStepper[Int] with StepperLike[Int, IncStepperA] {
  ...
  class IncStepperB(private val size0: Long) extends TryStepper[Int] with StepperLike[Int, IncStepperB] {
    if (size0 < 0) throw new IllegalArgumentException("Size must be >= 0L")
    private var i = 0L
    def characteristics = 0
    def knownSize = math.max(0L, size0 - i)
